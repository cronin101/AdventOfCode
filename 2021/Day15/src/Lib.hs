...
-- >>> solveDynamic <$> loadInput "example.txt"
...
solveDynamic :: Landscape -> SolveState
solveDynamic landscape = solve initialState [(0, 0)]
  where
    solve :: SolveState -> [Coordinate] -> SolveState
    solve state [] = state
    solve state previouslyUpdated = solve (M.union (M.fromList nextUpdates) state) $ map fst nextUpdates
      where
        frontier = S.toList $ S.unions $ map adjacentsTo previouslyUpdated
        nextUpdates = filter isDifferent $ map (ap (,) nextValue) frontier
        isDifferent (point, nextVal) = nextVal /= currentValue point
        adjacentsTo p = fromJust $ M.lookup p adjacentMap
        currentValue p = fromJust $ M.lookup p state
        nextValue point = case currentValue point of
          Just value -> Just $ minimum (value : options)
          _ -> Just $ minimum options
          where
            Just enterCost = M.lookup point landscape
            options = map (+ enterCost) $ mapMaybe currentValue $ S.toList $ adjacentsTo point
