...
part1 :: World -> Int
part1 =
  let trailsToTop = S.toList . S.filter ((== 9) . height) . trails
      uniqueEndingsByStart = M.unionsWith S.union . map (\t -> M.singleton (start t) (S.singleton $ end t))
   in sum . map S.size . M.elems . uniqueEndingsByStart . trailsToTop . solve
